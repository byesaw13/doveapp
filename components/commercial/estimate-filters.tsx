...
interface EstimateFiltersProps {
  ...
  onFiltersChange: (filters: EstimateFiltersState) => void;
  initialFilters?: Partial<EstimateFiltersState>;
  className?: string;
}

const statusOptions: Array<{ value: EstimateStatus | 'all'; label: string }> = [
...
export function EstimateFilters({
  ...
  const [filters, setFilters] = React.useState<EstimateFiltersState>({
    ...
    status:
      (searchParams.get('status') as EstimateStatus | 'all') ||
  ...
  const updateURL = React.useCallback(
    (newFilters: EstimateFiltersState) => {
  ...
  const handleFilterChange = React.useCallback(
    <K extends keyof EstimateFiltersState>(
      key: K,
  ...
  const clearFilters = React.useCallback(() => {
    const cleared: EstimateFiltersState = {
      search: '',
      status: 'all',
      clientId: '',
      dateFrom: undefined,
    ...
    <div className={cn('space-y-3', className)}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select
          ...
          onValueChange={(v) =>
            handleFilterChange('status', v as EstimateStatus | 'all')
          }
        >

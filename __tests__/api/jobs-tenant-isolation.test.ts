...
describe('/api/jobs tenant isolation', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('scopes job listing to the authenticated account', async () => {
    const mockContext = {
      accountId: 'account-a',
      userId: 'user-a',
      role: 'OWNER' as const,
    ...
    await GET(new NextRequest('http://localhost:3000/api/jobs'));

    expect(query.eq).toHaveBeenCalledWith('account_id', 'account-a');
  });

  it('writes jobs only within the authenticated account', async () => {
    const mockContext = {
      accountId: 'account-a',
      userId: 'user-a',
      role: 'ADMIN' as const,
    ...
    mockValidateRequest.mockResolvedValue({
      data: { client_id: 'client-1', title: 'New Job' },
      error: null,
    } as any);

    const request = new NextRequest('http://localhost:3000/api/jobs', {
      method: 'POST',
      body: JSON.stringify({ client_id: 'client-1', title: 'New Job' }),
